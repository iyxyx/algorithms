...
// 队列初始化
void initQueue1(Queue* q) {
    q->front = q->rear = 0;
}

// 入队（BFS核心步骤之一）
bool enQueue1(Queue* q, int val) {
    if ((q->rear + 1) % QUEUE_SIZE == q->front) return false; // 队列满
    q->data[q->rear] = val;
...
// 添加无向边（为BFS准备测试数据）
void addEdge1(Graph* g, int v1, int v2) {
